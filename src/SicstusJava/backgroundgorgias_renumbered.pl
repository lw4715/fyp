...
rule(bg68(),industry(X),[]) :- listIndustries(L),member(X,L).
%% rule(bg72,hasResources(X),[listHasResources(L),member(X,L)]).
%% rule(bg72b,neg(hasResources(X)),[listNegHasResources(L),member(X,L)]).
rule(bg69(),firstLanguage(chinese,X),[]) :- listChineseCountries(L),member(X,L).
rule(bg70(),firstLanguage(english,X),[]) :- listEnglishCountries(L),member(X,L).

%% part of nsa
%% have support of us gov/ has relations
rule(bg71(),isInfrastructure(nuclear),[]).
rule(bg72(),isInfrastructure(electricity),[]).
rule(bg73(),isInfrastructure(water),[]).
rule(bg74(),informationRich(banking),[]).
rule(bg75(),informationRich(infocomm),[]).
rule(bg76(),informationRich(consumer),[]).

rule(bg77(),possibleMotive(sabotage ,Att),[isInfrastructure(Ind),industry(Ind,V),target(V,Att)]).
rule(bg78(),possibleMotive(espionage ,Att),[informationRich(Ind),industry(Ind,V),target(V,Att)]).

%% rule(bg78,isCulprit(equationGroup ,flameattack),[]). 
rule(bg79(),malwareLinkedTo(flame,equationGrp),[]). 

%% rule(bg94,isCulprit([ united_states_of_america ,israel ],flameattack),[]).
rule(bg80(),target(middleeast ,flameattack),[]).
rule(bg81(),malwareUsedInAttack(flame ,flameattack),[]).
rule(bg82(),ccServer(gowin7 ,flame),[]).
rule(bg83(),ccServer(secuurity ,flame),[]).
rule(bg84(),domainRegisteredDetails(gowin7 ,adolph_dybevek ,prinsen_gate_6),[]).
rule(bg85(),domainRegisteredDetails(secuurity ,adolph_dybevek ,prinsen_gate_6),[]).
rule(bg86(),addressType(prinsen_gate_6 ,hotel),[]).

%% fireeye tech
rule(bg87(),fileCharaMalware(wannacry_filechara1,wannacry),[]).
rule(bg88(),fileChara('mssecsvcexe','db349b97c37d22f5ea1d1841e3c89eb4','3723264','2010-11-20T09:03:08Z','Loader+WormComponent','EXE',wannacry_filechara1),[]).
rule(bg89(),fileCharaMalware(wannacry_filechara2,wannacry),[]).
rule(bg90(),fileChara('taskscheexe','84c82835a5d21bbcf75a61706d8ab549','3514368','2010-11-20T09:05:05Z','Loader','EXE',wannacry_filechara2),[]).
rule(bg91(),fileCharaMalware(wannacry_filechara3,wannacry),[]).
rule(bg92(),fileChara('Unavailable','f351e1fcca0c4ea05fc44d15a17f8b36','65536','2009-07-1401:12:55Z','Encryptor','DLL',wannacry_filechara3),[]).
rule(bg93(),fileCharaMalware(wannacry_filechara4,wannacry),[]).
rule(bg94(),fileChara('@WanaDecryptor@exe','7bf2b57f2a205768755c07f238fb32cc','245760','2009-07-1323:19:35Z','Decryptor','EXE',wannacry_filechara4),[]).
...
rule(bg95(),gci_tier(X,initiating),[]) :- list_gci_initiating(L),member(X,L).
rule(bg96(),gci_tier(X,maturing),[]) :- list_gci_maturing(L),member(X,L).
rule(bg97(),gci_tier(X,leading),[]) :- list_gci_leading(L),member(X,L).
