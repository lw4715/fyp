...
rule(conflict, hasMotive(X, Att), [attackYear(Att, Y), target(T, Att), recentNewsInYear(N, T, Y), causeOfConflict(X, N), specificTarget(Att)]).
...
rule(sabotage(R), hasMotive(X, Att), [target(T, Att), attackYear(Att, Y),
  recentNewsInYear(R, T, Y), causeOfConflict(X, T, R), specificTarget(Att)
  ]).
